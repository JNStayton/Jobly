	...
	static async get(handle) {
		const companyRes = await db.query(
			`SELECT c.handle,
				  ...
				  array_agg(json_build_object('id', j.id, 'title', j.title, 'salary', j.salary, 'equity', j.equity, 'companyHandle', j.company_handle))
				  FILTER (WHERE j.company_handle IS NOT NULL) AS jobs
           FROM companies AS c
